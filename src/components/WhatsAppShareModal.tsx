...
// Derives real analysis from actual document data — no fabrication
function getRealAnalysis(doc: Document) {
  const clauses = doc.clauses ?? [];

  // Worst clause risk wins: High > Medium > Low
  ...
  // Simple deterministic mapping from real risk level — no fake math
  const riskScore  = riskLevel === 'High' ? 75 : riskLevel === 'Medium' ? 45 : 20;
  const trustScore = 100 - riskScore;

  return { riskLevel, riskScore, trustScore };
}

function buildShareMessage(doc: Document): string {
  const {riskLevel, riskScore, trustScore} = getRealAnalysis(doc);
  const riskEmoji =
...
${riskEmoji} *Risk Level:* ${riskLevel} (${riskScore}%)
...
${clauseLines || '• No clauses extracted.'}
...
export function WhatsAppShareModal({ document: doc, onClose }: WhatsAppShareModalProps) {
  ...
  const { riskLevel, riskScore, trustScore } = hasAnalysis
    ? getRealAnalysis(doc)
    : { riskLevel: 'Low' as const, riskScore: 0, trustScore: 0 };

  const riskStyle = RISK_STYLE[riskLevel];
  const message   = hasAnalysis ? buildShareMessage(doc) : '';
        ...
        {!hasAnalysis ? (
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 flex items-start gap-3">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
              Analysis not yet available. This document is still processing or
              failed to analyse. Sharing is disabled until analysis completes.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3 border border-gray-100 dark:border-gray-800">
              ...
              <div className="mt-2 h-1 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-700 ${riskStyle.bar}`}
